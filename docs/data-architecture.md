...
| Silver | `silver/sec/silver.duckdb`, `silver/sec/shards/shard-{shard_index}.duckdb`, `silver/fundamentals/shard-0.duckdb` | DuckDB staging and serving state. |
...
| MDM tracked-universe seeding | Automated + Prerequisite | `edgar-warehouse mdm seed-universe` | edgartools-mediated `edgar.get_company_tickers()`. | CIK, ticker, exchange, MDM entity shell, tracking status. | `mdm_entity`, `mdm_company`. Automated in `load_history` (the `MdmSeedUniverse` step, idempotent upsert, runs before window computation) so a fresh environment has a deterministic path from empty MDM tables to a runnable load — this step is also a hard prerequisite for `compute-windows`/`bootstrap-next`/`bootstrap-fundamentals` to find any CIKs at all. This is the normal control-plane source for bootstrap scope. |
| Window computation | Automated | `compute-windows`, `write-run-summary` | Internal MDM CIK list and prior bronze window files. | Window offset/limit, CIK snapshot, window count, CIK count. | `cik_windows.jsonl`, `cik_snapshot.jsonl`, `run-summary.json`. |
...
| Gold build and export | Automated | `gold-refresh`, gold-affecting warehouse commands | Internal silver and fundamentals shards. | Dimension keys, fact keys, filing/ownership/adviser/financial/fundamental measures. | S3 gold Parquet, S3 Snowflake export Parquet, Snowflake run manifest. |
