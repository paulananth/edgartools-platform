...
| Submissions bootstrap | Automated | `bootstrap`, `bootstrap-full`, `bootstrap-next`, `bootstrap-batch` | Direct SEC submissions JSON unless cached bronze is available. Scope comes from explicit CIKs or silver `sec_company_sync_state.tracking_status`. | Company profile, addresses, former names, submission pagination files, filing metadata. | Bronze submissions JSON, `sec_company`, `sec_company_address`, `sec_company_former_name`, `sec_company_submission_file`, `sec_company_filing`, `sec_company_sync_state`. |
