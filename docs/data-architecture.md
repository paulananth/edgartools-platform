...
| edgartools-mediated | The repository delegates lookup or parsing to the `edgartools` package. The original source remains SEC EDGAR unless noted. | `edgar.get_by_accession_number`, `edgar.get_company_tickers`, `EarningsRelease`, proxy summary compensation extraction, 13F information table parser. |
...
| Ownership parsing | Automated | Automatic parser policy for Forms 3/4/5, `parse-ownership-bronze` | Local `ownershipDocument` XML parser over the cached artifact, reporting owners classified from the owner CIK's bronze `submissions.json`; zero SEC calls (before ticket 19 the edgartools path fetched every owner's submissions live). | Reporting owner CIK/name/roles/title, non-derivative transactions, derivative transactions, shares, price, codes, ownership nature/directness, exercise/expiration/underlying fields. | `sec_ownership_reporting_owner`, `sec_ownership_non_derivative_txn`, `sec_ownership_derivative_txn`, `sec_parse_run`. |
| ADV parsing | Manual/backfill-only | `parse-adv-bronze --artifact` | Operator-staged ADV/IAPD/FOIA XML in bronze, named per artifact; local parser. ADV is not captured by normal SEC EDGAR bootstrap. | Adviser name, SEC file number, CRD/IARD number, effective date, filing status, offices, disclosures, private funds, AUM. | `sec_adv_filing`, `sec_adv_office`, `sec_adv_disclosure_event`, `sec_adv_private_fund`. Scheduled ADV ingestion is `fetch-adv-bulk` + `ingest-relationship-sources` (filings and private funds only); this command is for hand-staged XML. |
...
| Targeted resync/repair | Manual/backfill-only | `targeted-resync` | A specific, operator-identified CIK/accession/reference scope; optional artifact/text/parser refresh. | Force-refetched submissions, artifacts, or reference data for that scope. | Repaired bronze/silver/parser rows for the targeted scope. Own standalone Step Functions (`targeted_resync`), not chained into the main load/incremental pipelines. (`full-reconcile` — the SEC-drift *detection* tool that used to feed this repair path with candidates — was decommissioned entirely: zero executions ever, no schedule. Targeted resync now requires an operator to already know what to repair.) |
| Silver shard migration and cached replays | Manual/backfill-only | `migrate-silver-shards`, `seed-silver-batches`, `seed-bronze-batches`, `silver_mdm_gold`, `one_click_data_refresh` state machines | Internal silver or existing S3 bronze only; designed to avoid new SEC calls when artifact policies are skipped. | CIK batch lists, shard manifests, shard DuckDB files. | Sharded silver DuckDB, replayed MDM/gold/Snowflake exports. Operator-triggered recovery/replay tools, run on demand. |
| MDM entity resolution | Automated | `mdm mastering --entity-type ...` | Internal silver DuckDB or sharded reader. | Company, adviser, person, security, fund entities, source references, rule outcomes, survivorship/stewardship state. | `mdm_entity`, `mdm_company`, `mdm_adviser`, `mdm_person`, `mdm_security`, `mdm_fund`, source refs and curation tables. |
| MDM relationship derivation | Automated | `mdm derive-relationships`, `mdm load-relationships`, `mdm infer-relationships` | Internal MDM plus silver source tables. | `IS_INSIDER`, `HOLDS`, `COMPANY_HOLDS`, `ISSUED_BY`, `IS_ENTITY_OF`, `HAS_PARENT_COMPANY`, `MANAGES_FUND`, `IS_PERSON_OF`, `EMPLOYED_BY`, `AUDITED_BY`, `INSTITUTIONAL_HOLDS`. | `mdm_relationship_instance`, graph-ready sync tables. `infer-relationships` (previously `backfill-relationships`) is the form chained into every automated MDM chain; `derive-relationships`/`load-relationships` are manual/ad-hoc entry points onto the same underlying logic. |
| MDM Snowflake export and graph sync | Automated | `mdm publish`, `mdm publish-relationships`, `mdm reconcile` | Internal MDM SQL state and Snowflake connection settings. | Current MDM entity rows, graph nodes, graph edges, graph parity counts. | Snowflake MDM tables and `NEO4J_GRAPH_MIGRATION` graph-ready node/edge tables. `mdm publish` is a required prerequisite for `mdm publish-relationships`, not optional or manual-only: every automated AWS MDM chain (`load_history`, `bootstrap`, `daily_incremental`, `silver_mdm_gold`, `one_click_data_refresh`, `mdm_gold`, `ownership_mdm_gold`) runs publish immediately before publish-relationships so the Snowflake MDM mirror reflects the run/infer-relationships pass that just completed. `reconcile` is validation-only (Optional/validation) — it never blocks `gold-refresh` on failure to run, only reports parity. |
...
| Table/output | Source mode | Data points | Loader / Date Range |
| --- | --- | --- | --- |
| `sec_ownership_reporting_owner` | local XML parser over cached artifact + bronze `submissions.json` | Accession, owner index, owner CIK/name (display and raw registry form), director/officer/10-percent/other flags, officer title, `other_text`, filing footnotes/remarks, two address booleans, submissions structural evidence, parser version. | Forms 3/4/5 lookback, default **2 years** (see Filing Artifacts and Text above). |
| `sec_ownership_non_derivative_txn` | local XML parser over cached artifact | Accession, owner index, transaction index, security title, transaction date/code, shares, price, acquired/disposed code, shares owned after, ownership nature/directness, parser version. | Same, **2 years**. |
| `sec_ownership_derivative_txn` | local XML parser over cached artifact | Non-derivative fields plus conversion/exercise price, exercise/expiration dates, underlying security title/shares. | Same, **2 years**. |
...
- Ownership Forms 3/4/5: only `edgar.display.formatting.reverse_name` and `edgar.entity.constants._classify_is_individual`, applied to bronze `submissions.json`; the XML itself is parsed locally.
...
All chains below run the same MDM sequence — `mdm mastering` -> `mdm infer-relationships`
-> `mdm publish` -> `mdm publish-relationships` -> `mdm reconcile` (referred to as "MDM chain"
below) — with `mdm publish` immediately before `mdm publish-relationships` so the Snowflake MDM
mirror `sync-graph` reads is never stale relative to the run/infer-relationships pass that just
completed.
...
| `one_click_data_refresh` | List CIKs from existing S3 bronze -> cached `bootstrap-batch` -> MDM chain -> `gold-refresh`; intended for cold-start/recovery from bronze. |
...
| MDM utility workflows | `mdm migrate`, `check-connectivity`, `mastering`, `infer-relationships`, `publish-relationships`, `reconcile`, `counts`, `seed-universe`. Note: `mdm publish` has no standalone utility workflow of its own today — it only runs embedded in the MDM chains above. `seed-from-silver` retains its CLI command but no longer has a dedicated state machine (state-machine-consolidation wayfinder map, ticket 04 — zero callers of any kind, its original AWS-RDS-VPC-access rationale is stale post-Snowflake-Postgres-migration); run it via a one-off `aws ecs run-task` or locally if ever needed. |
