...
| Deterministic result | Same evidence, policies and steward decisions replayed with record permutations, batch sizes, duplicate deliveries and worker scheduling; equal identities, fields, aliases, edge sets and business hashes |
...
| Field provenance | Equal-ranked disagreement has a stable winner; source assertion, policy digest, origin run and losing evidence remain queryable |
...
| Reversal | Wrong merge followed by valid field correction, relationship and dependent merge; preview/replay restores partitions without losing later evidence; ambiguous facts are deferred |
