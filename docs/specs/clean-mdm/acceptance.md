...
| Deterministic result | Same evidence, policies, steward decisions and retained identity registry replayed with record permutations, batch sizes, duplicate deliveries and worker scheduling; equal identities, fields, aliases, edge sets and business hashes. Registry-preserving replay is the revised proposal pending the ID-stability decision. |
...
| Field provenance | Equal-ranked disagreement has a stable winner; coherent field groups retain one source claim; source assertion, policy digest, origin run and losing evidence remain queryable; override expiry and contradictory source updates follow approved policy |
...
| Reversal | Wrong merge followed by valid field correction, relationship and dependent merge; preview/replay restores partitions without losing later evidence; ambiguous facts are deferred; proposed Match Exclusion prevents replay from recreating the rejected merge |
