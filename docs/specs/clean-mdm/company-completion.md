...
| 3 | Implement shared GLEIF capture and normalization | Stream native JSON ZIP (captured format) or explicitly pinned XML ZIP; retain complete source records and unsupported kinds; normalize eligible Company evidence; no direct master writes; every input has an accounted disposition |
