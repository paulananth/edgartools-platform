...
- Implementation of Ticket 02's locked design (checkpoint table + migration
  + per-type wiring across the 7 in-scope types) — needs its own execution
  ticket(s) before someone can build it, mirroring
  [state-machine-consolidation](../state-machine-consolidation/map.md)'s own
  "map decides, execution tickets implement" split. Not yet specified into
  a ticket this session — the one piece of code actually touched this
  session (the `EMPLOYED_BY` `remaining=None` bug) was a narrow,
  independent correctness fix, not part of this design's implementation.
