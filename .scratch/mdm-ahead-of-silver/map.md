...
- [Narrow the Backfill Sweep's Storage Target](issues/06-narrow-backfill-storage-target.md) — supersedes Ticket 02's "both targets": Snowflake only (DuckDB drops out entirely, both as read and write source), full-row re-emission (not a thin key-only append — the dbt collapse is latest-`parse_sequence`-wins per row, so a thin append would null out every other column). `parse_sequence` is a Snowflake sequence default, confirmed not a blocker for re-emission always outranking the row it supersedes. Drops the `sec_fetch_active` lease requirement and all shard-hydrate machinery `47fe8fb5` added — that commit is a correct, tested intermediate state, not a mistake; superseding changes land as a follow-up, not a revert.
