...
- [Verify Observability and Image Cost Controls](issues/06-verify-observability-and-image-cost-controls.md) — PR #401 merged (`13f5ad92`) and deployed live to `edgartools-prod`: 7-day retention on all 3 log groups confirmed to survive a real redeploy (the exact drift ticket 01/03 caught), first-ever ECR rollback-registry cohort recorded and its dry-run plan correctly fail-closed to "protect everything" (0 deletable, expected with only 1 of 3 required cohorts), current+mdm images confirmed pullable and launchable via a live ECS `run-task` smoke test. Log-volume-vs-baseline re-measurement deliberately left as a follow-up against the next real production execution rather than fabricated from a smoke test.
