# 07 — Complete the Local Test-Suite Port to SQLite
...
This ticket is the cleanup pass: after Tickets 01–05 and the external
`dbt-gold-silver-rewiring` chain land (each carrying its own test changes as
part of its own work), re-survey the DuckDB-touching test file list and port
whatever operational-bookkeeping coverage is still left uncovered to SQLite.
Don't re-port tests that another ticket already rewrote as part of its own
scope — this ticket closes the remainder, not the whole 56.
...
[Ticket 05](05-retire-ddl-generator-scripts.md), and the
...
- [ ] Re-survey the 56-file DuckDB-touching test list against what Tickets
      01–05 and the `dbt-gold-silver-rewiring` chain already closed
