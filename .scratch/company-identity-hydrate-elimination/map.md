...
- [Does AWS Step Functions Distributed Map redrive actually resume only failed batches for this delta-then-reduce shape?](issues/04-verify-distributed-map-redrive.md) — No, not as this repo is wired today (verified against AWS's primary docs/launch blog, not inferred). Redrive genuinely skips succeeded children and reruns only failed ones for `STANDARD`-type Distributed Map children (matches this repo's shape) — but AWS explicitly excludes errors "handled ... using catch, retry, and routing to a Fail state" from redrive eligibility. This repo's `sec_fetch_task_catch()` (wired on every Stage0 Map and `ReduceIdentityRefresh`, added by release-readiness ticket 86 to release the `sec_fetch_active` lease promptly instead of waiting out an 18h stale-reclaim) routes every failure to a terminal `Fail` state — exactly the disqualifying pattern. A redrive today would just re-enter and re-fail `SecFetchTaskFailed`, not resume the other ~52 successful batches. Redrive is also always manual (no auto-trigger). Recommended fallback: an explicit CLI-level partial-resume path on top of the existing manifest/outcome contract, rather than depending on redrive or reopening ticket 86's lease-release fix.
- [Decide the target architecture for Stage0CompanyIdentity's windowed silver read/write](issues/03-decide-target-architecture.md) — **Map destination reached.** Locked: (1) selective/minimal-table hydrate (fixes the OOM), (2) restructure to delta-then-reduce mirroring daily_incremental's bounded Identity Refresh (fixes the ~2hr repeated-I/O cost), (3) `reduce_identity_refresh`'s per-candidate disk-accumulation bug fixed first, as its own standalone prerequisite, (4) failure isolation via an explicit CLI-level partial-resume path on the manifest/outcome contract (not SFN redrive, verified inapplicable; not a change to ticket 86's lease-release Catch), (5) scope is `load_history` only. Implementation is out of this map's scope per its Notes — a follow-up (non-wayfinder) session builds this.
