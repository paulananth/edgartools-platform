...
7. [Profile gold-refresh stage breakdown](issues/07-profile-gold-refresh-stage-breakdown.md) — resolved: triggered a fresh run (both historical runs predated current fixes and their logs had expired). Breakdown of 169.12s total: silver DB hydration 8.2% (13.78s), setup gaps 4.5%, gold table build (27 tables) 33.0% (55.77s), **silver merge/publish (unconditional) 35.9% (60.65s) — the single largest cost, and a genuine no-op** (gold-refresh never writes to silver; the observed run's own event confirms canonical/source/staged checksums were all identical), container overhead 18.5%. The no-op-republish finding is bigger and more clearly actionable than a fan-out question — split off as [ticket 10](issues/10-decide-gold-refresh-unconditional-silver-republish.md) (grilling, since the no-`--force` design looks deliberate and needs understanding before touching it, not just skipping). Unblocks ticket 08 (fan-out) using the 55.77s/27-table build breakdown, though 08's cost/benefit should be re-evaluated once ticket 10 resolves.
