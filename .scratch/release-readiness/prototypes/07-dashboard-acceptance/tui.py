...
def glyph(status: str) -> str:
    return {"pass": f"{GREEN}✔{RESET}", "fail": f"{RED}✘{RESET}", "not_checked": f"{DIM}·{RESET}"}[status]
...
def render(state: logic.DashboardAcceptanceState) -> None:
    ...
    print(f"{DIM}release_candidate={RESET} {state.release_candidate}   "
    ...
    for i, (dash, vid, label) in enumerate(logic.VIEWS, start=1):
        key = f"{dash}::{vid}"
        c = state.views[key]
        flags = []
        if key in stale:
        ...
        print(f"{i:2d}. {glyph(c.status)} [{dash:20s}] {label}{flag_str}")
    print()
    print(f"{BOLD}Overall: {logic.overall_status(state)}{RESET}")
    print()
...
def main() -> None:
    state = logic.init_state(release_candidate="rc-20260729-e0fa0eaafb09", release_watermark="wm-2026-07-29T02:00Z")
    render(state)
    while True:
        try:
            line = input("> ").strip()
        ...
        if cmd == "q":
            ...
            idx = int(parts[1]) - 1
            dash, vid, _label = logic.VIEWS[idx]
            key = f"{dash}::{vid}"
            state = logic.record_check(
                state, key, status="pass", watermark_checked=state.release_watermark,
            ...
            key = f"{dash}::{vid}"
            state = logic.record_check(
                state, key, status="fail", watermark_checked=state.release_watermark,
                operator="dashboard_reviewer", mutation_surface_clear=True,
                secret_leakage_clear=True, unbounded_output_clear=True,
