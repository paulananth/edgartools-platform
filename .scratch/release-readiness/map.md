...
- [Fix reduce_identity_refresh double-fetch](issues/76-fix-reduce-identity-refresh-double-fetch.md) — open: found via a fresh `/gof-refactor-reviewer` pass while resolving pipeline-throughput-architecture's ticket 05 — `reduce_identity_refresh` fetches every reference/delta object from S3 twice per attempt (once to verify checksum, bytes discarded; once again inside the merge loop to actually use). Low-risk fix: keep the bytes from the first fetch. Not yet implemented.
