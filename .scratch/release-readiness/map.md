...
- [Implement shared submissions-fetch concurrency](issues/78-implement-shared-submissions-fetch-concurrency.md) — open: implements pipeline-throughput-architecture ticket 06's decision (urgent — `bootstrap-batch`'s existing 3-task fan-out risks up to 27 req/sec against SEC's 10 req/sec ceiling). `ThreadPoolExecutor` on the shared `_capture_submission_bronze_snapshot`, covers 5 commands at once, plus `BOOTSTRAP_BATCH_CONCURRENCY` re-evaluation. Not yet implemented.
