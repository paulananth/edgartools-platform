...
6. [Add Catch on RunWarehouseTask to release sec_fetch_active on failure](issues/86-add-catch-to-runwarehousetask-sec-fetch-lease.md) — task, real gap: live ticket-84 verification wedged the lease twice via two unrelated mundane failures, since `RunWarehouseTask` has no `Catch` in any of the 5 lease-wired machines. The `adv_bulk_fetch_catch` pattern already exists in the same file for a different state; needs the same treatment here. Recommend before next scheduled `daily_incremental`.
