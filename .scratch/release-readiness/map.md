...
4. [Fix CLAUDE.md's stale Phased Pipeline concurrency documentation](issues/66-fix-stale-phased-pipeline-concurrency-docs.md) — resolved 2026-08-04: rewrote the Phased Pipeline section to match the live 5-Map `MaxConcurrency=1` shape (re-verified live, not from the ticket's older findings), removed the unverified timing claim, reworded the `bootstrap-next` guidance to its real current reason, and clarified which two pipelines (`silver-mdm-gold`, real; `bootstrap-batched`, unverified/unused) the `BOOTSTRAP_BATCH_CONCURRENCY` invariants actually govern.
5. [Fix invalid-escape-sequence SyntaxWarning in gold_models.py](issues/85-fix-invalid-escape-sequence-syntaxwarning-gold-models.md) — resolved, merged to main.
8. [Investigate missing S3 object for cached accession text extraction](issues/88-missing-s3-object-for-cached-accession-text-extraction.md) — implemented and tested 2026-08-04, not yet deployed. Root cause: NOT isolated -- 494 of Apple (CIK 320193)'s 1,044 `sec_raw_object` rows (47%) reference S3 keys that don't exist, most likely one-off manual/repair scripts (2026-07-25, 2026-07-31), not the standing pipeline. Fix: `fetch_filing_artifacts` and `extract_text_for_accession` now verify S3 presence (one LIST per accession, gated to avoid cost on cold accessions) before trusting a cache hit, self-healing via a real re-fetch instead of crashing. Live-verified the presence-check logic itself against real prod S3 (exact string match on 5 present objects, correct absence on the known-missing one). Full suite green (1322 passed). Backfilling the 494 known-missing objects and scanning other CIKs split to tickets 90/91.
9. [`daily_identity_refresh` lease stuck, no release-on-failure](issues/89-daily-identity-refresh-lease-stuck-no-catch.md) — root cause found and unblocked live 2026-08-04: today's `daily_incremental` deferred and did zero work because a *different* lease than `sec_fetch_active` (`daily_identity_refresh`) was `held` since 2026-08-03 21:57 UTC by ticket 83's own OOM incident (`reduce-identity-refresh`, exit 137). Cleared immediately via the real `release-identity-refresh-lease` ECS task (not a raw DB mutation) rather than waiting for the ~17:57 UTC 20h staleness backstop; a fresh `daily_incremental` run was retriggered. Same gap class as ticket 86 (no `Catch` to release-on-failure) but for a separate lease ticket 86 didn't cover -- the structural fix (Catch in `deploy-aws-application.sh`) is not yet implemented, deliberately deferred (advisor: out of scope for this session, only matters after a deploy).
10. [Backfill Apple's 494 missing bronze objects](issues/90-backfill-apple-494-missing-bronze-objects.md) — in_progress: operator chose targeted-resync (option b); armed and waiting on `daily_incremental`'s `sec_fetch_active` lease to free, not yet fired as of 2026-09-12.
11. [Scan other CIKs for the same missing-bronze-object pattern](issues/91-scan-other-ciks-for-missing-bronze-objects.md) — resolved 2026-09-12: targeted scan (rows fetched on the 2 suspect repair-script dates, all CIKs) found the pattern is NOT Apple-specific — 7 more mega-cap CIKs affected (Coca-Cola, Tesla, Microsoft, Nvidia, Berkshire Hathaway, Amazon, Alphabet), 1,274 more missing objects beyond Apple's 494. Follow-up backfill scoped as [Ticket 103](issues/103-backfill-mega-cap-missing-bronze-objects.md).
6. [Add Catch on RunWarehouseTask to release sec_fetch_active on failure](issues/86-add-catch-to-runwarehousetask-sec-fetch-lease.md) — resolved: implemented, tested, deployed to prod, and live-verified 2026-08-04. A real `targeted-resync` failure (an unrelated missing-S3-object bug, ticket 88) hit the new Catch live: confirmed via direct canonical-S3 read that `sec_fetch_active` released cleanly (`status='idle'`, `run_id` matching the execution) with zero manual intervention -- unlike the two manual recoveries needed during ticket 84's verification before this fix existed.
7. [Investigate Apple Form 4 immutable-object content conflict](issues/87-immutable-object-conflict-apple-form4.md) — resolved: deployed to prod and live-verified 2026-08-04. Re-running the exact `targeted-resync --scope-type cik --scope-key 320193` that originally failed now isolates conflicts correctly -- **27 separate immutable-object conflicts** were caught and skipped in one run (far more than the single instance originally found, confirming this is a real, recurring SEC-side drift pattern for this CIK, not a one-off). Surfaced a new, unrelated bug along the way: ticket 88.

- [Fix sec_filing_attachment raw_object_id ambiguous conflicts blocking publication](issues/97-fix-filing-attachment-raw-object-id-conflicts.md) — resolved: found live via ticket 42's sample backfill retry (147 conflicts, exit code 2) — `sec_filing_attachment` declared no `authority_column` (no timestamp column exists in its DDL at all), so `raw_object_id` drift on SEC refetch was unconditionally ambiguous, same phenomenon `sec_raw_object`'s existing `provenance_columns` pattern already handles. Grilled decision: exclude `raw_object_id` from comparison (mirrors `sec_raw_object`, no schema migration) rather than add a real `fetched_at` authority column + backfill against ~3M live rows — confirmed nothing downstream reads this column. Two new regression tests, full suite green. Merged (PR #354), deployed to prod, and **live-verified 2026-08-05**: re-ran ticket 42's exact 20-CIK sample backfill end-to-end for the first time across 4 attempts — exited 0, `sec_filing_attachment` merged 8,212 inserted/0 conflicts, every other protected table clean too.
...
- [Implement cross-command SEC-fetch lease](issues/80-implement-cross-command-sec-fetch-lease.md) — in_progress: implements pipeline-throughput-architecture ticket 09's decision. Real overlap confirmed (bootstrap + daily-incremental, 4.16h, 2026-07-30). Phase 1 (DB primitive: `sec_fetch_active` lease name, `acquire-sec-fetch-lease`/`release-sec-fetch-lease` commands, own `lease_result.json` path template) done and tested (7 tests), full suite green. Verified ticket 79 does not regress the existing identity-refresh lease (`pipeline_run_lease` is dual-registered but `PROTECTED_TABLE_REGISTRY` wins, so it always survives merge). Phase 2 (Step Functions wiring across the 5 state machines — the actual enforcement) split to ticket 84 per advisor review.
- [Wire sec-fetch lease into state machines](issues/84-wire-sec-fetch-lease-into-state-machines.md) — in_progress: ticket 80's Phase 2, resolved via grilling (fetch-heavy-phases-only boundary, defer-and-terminate semantics, 16h staleness window sized against real measured `bootstrap`/`daily_incremental` runtimes). `daily_incremental`/`bootstrap`/`bootstrap_full`/`targeted_resync`/`load_history` all wired and tested, full suite green, merged to `main` via PR #349, deployed to prod, and live-verified 2026-08-04 -- rebuilt both prod images first (found 4/49 commits stale respectively), confirmed via real overlapping `targeted-resync` executions that the second genuinely defers (`lease_acquired: false`, `held_by_run_id` matching the first execution exactly). Surfaced 2 real follow-ups: ticket 86 (`RunWarehouseTask` has no `Catch`, so any failure wedges the lease platform-wide -- hit twice live during verification, both manually recovered and confirmed via a direct canonical-S3 read, not just exit code) and ticket 87 (a genuine immutable-object content conflict on Apple's Form 4 accession `0000320193-24-000075`, unrelated to the lease work). Mid-session correction: `load_history` turned out to be sequential now (no fan-out), not the parallel `bootstrap-batch ×N` machine this ticket's text described -- that lives in a separate, still-deployed but undocumented `bootstrap_batched` machine, left unwired (see the ticket's "Not yet specified").
- [Implement skip-noop-silver-publish](issues/79-implement-skip-noop-silver-publish.md) — in_progress: implements pipeline-throughput-architecture ticket 10's decision via `compute_silver_fingerprint` (local-only table-set + per-protected-table row-count/content-hash), a hydration-time sidecar, and a pre-merge skip check in `_publish_silver_database_if_remote`. Code + all 4 test-plan cases (incl. advisor-added unregistered-table guard case) done and green, full suite green (1713 passed). Live measurement (item 5, the actual 60.65s→~cheap-check drop against real prod `gold-refresh`) not yet run.
- [reduce-identity-refresh OOM on silver merge](issues/83-reduce-identity-refresh-oom-on-merge.md) — in_progress: found live while chasing ticket 77/78's live-measurement run — `daily-incremental-ticket77-livemeasure-1785794199` OOM-killed (exit 137) in `ReduceIdentityRefresh` on `edgartools-prod-medium` (4096MB), mid-merge of `sec_company_filing`. Root-caused via a real `docker run --memory=4096m` reproduction against actual prod inputs (macOS-local RSS measurements had NOT shown the failure): ticket 76's whole-call `verified_bytes` dict stacks with a per-attempt full canonical re-read plus the merge's own working set. Fixed structurally (disk-cached verified inputs via `tempfile.mkdtemp` instead of an in-memory dict, cleaned up in `finally`) plus a belt-and-suspenders resize (`ReduceIdentityRefresh` moved medium→large, matching the gold-build-memory-reliability precedent). 3 new regression tests (2 unit, 1 architecture), full suite green (1724 passed, only the pre-existing unrelated `test_go_live_wizard.py` failure). Live prod verification (rebuild/deploy/fresh `daily-incremental` run) not yet done — that same run would also finally obtain ticket 77/78's live measurement. Blocks ticket 82/49/61/63's shared "fresh execution past the deploy" evidence gate.
- [Implement shared submissions-fetch concurrency](issues/78-implement-shared-submissions-fetch-concurrency.md) — in_progress: `ThreadPoolExecutor` (bound 5) on CIK-level submissions bronze-capture (scope sharpened from function-internal to caller-level to match the actual measured bottleneck), covers all 5 SEC-fetching commands, code + test plan items 1-4 done and green. `BOOTSTRAP_BATCH_CONCURRENCY=1` recommended but not yet applied. Live measurement (item 5) blocked on the same prod-deploy + ticket 74 run as ticket 77.
- [Implement artifact-fetch concurrency](issues/77-implement-artifact-fetch-concurrency.md) — in_progress: `ThreadPoolExecutor` (bound 5) on the per-document artifact-fetch loop, DB writes serialized to main thread, code + test plan items 1-4 done and green. Live measurement (item 5) blocked on prod deploy + ticket 74's repair-verify run finishing (ticket 09 mutual exclusion).
- [Fix reduce_identity_refresh double-fetch](issues/76-fix-reduce-identity-refresh-double-fetch.md) — resolved: `reduce_identity_refresh` now reads the reference snapshot and every batch delta exactly once for the whole call (captured into a `dict[path, bytes]` from the pre-loop verification pass, reused by both the merge loop and the no-baseline branch) instead of re-fetching per attempt — better than the ticket's "once per attempt" bar, since retries on `PromotionConflictError` now reuse the same bytes too. New test forces a conflict retry and counts reads by path; confirmed to fail pre-fix (3 reads each) and pass post-fix (1 each). Full suite green (1271 passed).
- [Batch daily_artifact_resume existence checks](issues/75-batch-daily-artifact-resume-existence-checks.md) — resolved: replaced `_exists_json`'s per-candidate, per-status `GetObject` with one `StorageLocation.find_existing` glob listing per `prepare_resume` call (`outcomes/*/*.json`), parsed locally into an `accession -> {statuses}` dict. Two new tests (call-count assertion confirmed to fail pre-fix, plus a mixed-state categorization equivalence check); both pre-existing tests pass unchanged. Full suite green (1272 passed).
- [daily-incremental permanent terminal-repair block](issues/74-daily-incremental-permanent-terminal-repair-block.md) — resolved (corrected 2026-09-12: all three "Done when" items settled). Item 1: root-caused to a one-byte (trailing newline) mismatch between legacy pre-2026-07-31T16:58 (pre-ticket-56 byte-exact fix) bronze content and freshly re-fetched raw SEC bytes for 2 CIK-2143673 accessions; no code path can overwrite an existing immutable object, so `record_repair_attestation` alone would not have unblocked the retry — directly corrected both stale bronze objects to byte-exact live content instead (S3 versioning enabled, old versions recoverable), verified byte-identical. Item 3: added `check_unresolved_terminal_repairs` (PR #610, `claude/gate-daily-incremental-on-terminal-repair-markers`) — a cheap up-front check at the very start of a recurring attempt, gated the same way `prepare_resume`'s own call site is, so a same-run_id retry with unresolved terminal-repair markers now fails in seconds instead of redoing ~95 minutes of daily-index/submissions-silver work. Item 2: operator decision — no proactive bucket scan; accepted as passive risk, mitigated by item 3's cheap fail-fast gate turning any future first-encounter into a cheap, quickly-repaired event rather than a multi-hour blind retry.
- [Fix daily-incremental failure alarm coverage gap](issues/81-fix-daily-incremental-failure-alarm-coverage-gap.md) — resolved: `configure_daily_incremental_alarms` (`deploy-aws-application.sh`) now also creates/deletes a real `ExecutionsFailed` alarm (`edgartools-prod-daily-incremental-failed`), mirroring the existing timeout alarm exactly (same topic, same guard, same flag). Deployed live to prod (PR #344) and confirmed via `describe-alarms`: new alarm exists, correctly dimensioned, `INSUFFICIENT_DATA` (expected, no datapoints yet). Orphaned `sec-edgar-pipeline-failure` alarm (dimensioned to a state machine confirmed not to exist) deleted after a final live re-check. `daily-incremental` now has real coverage for both timeout and application-level failures.
...
- [Clean up orphaned staged-promotion blobs in S3 `silverstage/`](issues/65-clean-up-orphaned-staged-promotion-blobs.md) — resolved: added `StorageLocation.delete_object` plus an explicit delete-on-success call in `reduce_identity_refresh` (never on `PromotionConflictError`, matching the documented leave-in-place-for-retry behavior), and an `aws_s3_bucket_lifecycle_configuration` on the `silverstage/` prefix (3-day expiration, shared dev+prod module). Merged as PR #339 (real conflict with PR #338/ticket 64 in `identity_refresh_publication.py`, resolved same as the earlier 77/78 conflict; full suite green after, 1270 passed). Swept the old `_staging/` prefix live: 48 objects/51.4GB deleted, confirmed empty. **New, not-yet-actioned finding:** the live `silverstage/` prefix already has its own smaller leak (16 objects, 18.4GB) since the code fix isn't deployed yet — deliberately not swept this pass (it's the still-actively-written prefix, some objects could be legitimate conflict-preserves); the lifecycle rule will catch up once deployed.
...
13. [Automate filing-text capture end-to-end](issues/101-automate-filing-text-capture-end-to-end.md) — blocked by 30 (resolved). Re-confirmed live 2026-09-07: `sec_filing_text`'s Snowflake schema now exists (built after ticket 30 was written) but still has 0 rows — the only writer, `targeted-resync --include-text`, has run 4 times ever in prod, all a month-old and unrelated to text capture. Trigger-condition sub-question resolved via `/grilling` 2026-09-07: not live/on-demand (the one stated consumer, `initiating-coverage`, fetches SEC directly and never touches this platform's data) — instead a ledger-driven background sweep folded into `daily_incremental`, `required` = genuine periodic-reporting CIK (Ticket 01's discriminator) with a periodic filing in the trailing 2 years AND a real ticker, `processed` = a `sec_filing_text` row exists for that CIK's current latest qualifying accession, both recomputed fresh every sweep. Cleanup of processed-but-no-longer-required artifacts is identify-only here (Tickets 70/71's split); deletion is a future follow-on. Snowflake export-path sub-question resolved 2026-09-07: traced the full chain (row shape, landing buffer, COPY INTO table list, dbt model) plus a real local end-to-end test producing a correctly-schemaed Parquet file — no gap found, already-proven mechanism, nothing blocks implementation. Parsing-at-scale sub-question resolved 2026-09-07: fetched 21 real 10-K/10-K405/10KSB/10KSB40 filings live from SEC spanning the iXBRL mandate boundary, found and root-caused a real defect — modern iXBRL filings' hidden `<ix:header>`/`display:none` XBRL-fact blocks leak into `BeautifulSoup.get_text()`'s output (no CSS engine, no XBRL awareness), contaminating the start of extracted text with taxonomy noise instead of business-description prose, for the majority of what this feature is meant to produce. Fix (strip those elements before `get_text()`) validated live, provably a no-op on pre-iXBRL filings; not yet implemented. Also found along the way: zero of 68,412 periodic-disclosure filings have any captured bronze attachment today — the sweep's first touch per company is a real SEC fetch, not a cache hit. Only schema-change sub-question (already answered: none needed) remains; ticket is otherwise implementation-ready.
14. [Decide whether to generalize the required/processed artifact ledger](issues/102-decide-general-artifact-required-processed-ledger.md) — blocked by 101. Surfaced while resolving 101: this is now the third instance of the same "required vs. processed vs. cleanup-candidate" shape in this codebase (ticket 63's `artifact_required` outcome ledger; tickets 70/71's binary-attachment cleanup; now 101's filing-text sweep) — deliberately not decided inline to avoid scope creep on an already-well-scoped question.
...
- **2026-09-05:** [Ticket 100](issues/100-adv-bulk-select-limit-plateau-on-restart.md) (adv_bulk.py's unordered `SELECT ... LIMIT N` plateau) resolved — confirmed live impact (`daily_incremental`'s daily `mdm mastering --entity-type all --limit 100` call forwards straight into `run_advisers`/`run_funds` with no growing-window exclusion, so this fires every day, not just theoretically). Fix picked up and tracked as [Ticket 06 on the `fundamentals-daily-integration` map](../fundamentals-daily-integration/issues/06-fix-adv-bulk-select-limit-plateau.md) instead of here, per user decision.
