...
- [Implement skip-noop-silver-publish](issues/79-implement-skip-noop-silver-publish.md) — open: implements pipeline-throughput-architecture ticket 10's decision. gold-refresh's silver-publish step is 35.9%/60.65s of its runtime for a structurally guaranteed no-op — skip via dynamic per-run detection of whether any protected table actually changed, not a static command allowlist. Not yet implemented.
