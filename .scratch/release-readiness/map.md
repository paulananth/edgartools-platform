...
5. [Fix invalid-escape-sequence SyntaxWarning in gold_models.py](issues/85-fix-invalid-escape-sequence-syntaxwarning-gold-models.md) — task (trivial, single-session: `'\s+'` should be `r'\s+'` at lines 583/659 of the `party_nk` owner-name-normalization SQL; not yet a functional bug, but Python will turn this into a hard `SyntaxError` in a future version. Found incidentally 2026-08-04 verifying the ticket 84 prod MDM image build, unrelated to that work.)
6. [Add Catch on RunWarehouseTask to release sec_fetch_active on failure](issues/86-add-catch-to-runwarehousetask-sec-fetch-lease.md) — in_progress: implemented and tested (10 new architecture tests, full suite green, all 5 generated definitions validated clean against real AWS). Scope grew from 1 to ~15 states across 3 factory functions once investigated (every currently-uncaught Task/Map state in each machine's fetch-heavy span, not just the literal `RunWarehouseTask`), confirmed via `AskUserQuestion` before implementing. Release-then-refail semantics (not silent success) to preserve `ExecutionsFailed`/alarm visibility. Not yet deployed to prod or live-verified.
7. [Investigate Apple Form 4 immutable-object content conflict](issues/87-immutable-object-conflict-apple-form4.md) — in_progress: root cause confirmed via direct SEC comparison -- a stable, one-byte trailing-newline drift on SEC's own served content (4141 vs 4142 bytes, same filing content otherwise), not a bug in this repo's byte-preserving capture path (confirmed via source read) and not a systemic pattern (3 other sampled Form 4s for the same CIK show zero drift). Fail-closed guard worked as designed. Still open: whether one whitespace-only conflict should abort the whole ~1000-accession CIK resync, or isolate to the one accession -- a real design tradeoff, not decided here.
