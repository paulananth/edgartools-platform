...
4. [Implement and Activate the Bounded Daily Identity Refresh Schedule](issues/49-implement-bounded-daily-identity-refresh-schedule.md) — task (graduated from resolved daily-incremental scope/cadence decision; implement the narrow daily path, weekly backstop, operator-owned schedules, cross-mode lease, manual evidence, and explicit activation checkpoint)
