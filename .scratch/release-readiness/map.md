...
- [Decide whether to exclude binary presentation artifacts from the default fetch policy](issues/70-decide-exclude-binary-artifacts-from-fetch-policy.md) — open, HITL — surfaced by advisor while resolving ticket 69: one live filing spent 83% of its fetch time (~4.3 of ~5.2s) downloading 24 investor-presentation JPGs nothing in this repo parses. Plausibly a bigger win than ticket 69, but changes what bronze captures going forward (operator trade-off, not an engineering call) — deliberately not bundled into the perf fix.
