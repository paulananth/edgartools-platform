...
- [Wire sec-fetch lease into state machines](issues/84-wire-sec-fetch-lease-into-state-machines.md) — in_progress: ticket 80's Phase 2, resolved via grilling (fetch-heavy-phases-only boundary, defer-and-terminate semantics, 16h staleness window sized against real measured `bootstrap`/`daily_incremental` runtimes). `daily_incremental`/`bootstrap`/`bootstrap_full`/`targeted_resync`/`load_history` all wired and tested, full suite green, merged to `main` via PR #349 (2026-08-04). Mid-session correction: `load_history` turned out to be sequential now (no fan-out), not the parallel `bootstrap-batch ×N` machine this ticket's text described -- that lives in a separate, still-deployed but undocumented `bootstrap_batched` machine, left unwired (see the ticket's "Not yet specified"). Live overlapping-trigger verification (test plan item 4) not yet run.
