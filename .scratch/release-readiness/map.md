...
- [Fix daily-incremental failure alarm coverage gap](issues/81-fix-daily-incremental-failure-alarm-coverage-gap.md) — in_progress: `configure_daily_incremental_alarms` (`deploy-aws-application.sh`) now also creates/deletes a real `ExecutionsFailed` alarm (`edgartools-prod-daily-incremental-failed`), mirroring the existing timeout alarm exactly (same topic, same guard, same flag). Contract tests extended, confirmed to fail pre-fix, full suite green (1274 passed). Not yet deployed live, and the orphaned `sec-edgar-pipeline-failure` alarm (recommend: delete) not yet retired — both are live prod AWS actions deliberately deferred for explicit confirmation.
