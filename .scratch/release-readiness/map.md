...
8. [Investigate missing S3 object for cached accession text extraction](issues/88-missing-s3-object-for-cached-accession-text-extraction.md) — task, found live 2026-08-04 during ticket 86/87's post-deploy verification: `targeted-resync` accession `0001140361-23-004439` logged an `accession_cache_hit` but the actual S3 object for its cached `raw_object_id` appears missing (`include_text` step failed with an `s3fs`/`fsspec` error trying to open it). Unrelated to tickets 86/87 -- not an immutable-object conflict. Not yet investigated: whether the row points at a never-written or deleted object, and whether this is isolated or systemic.
