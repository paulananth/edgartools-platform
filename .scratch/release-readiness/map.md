...
- [Wire sec-fetch lease into state machines](issues/84-wire-sec-fetch-lease-into-state-machines.md) — open, task: ticket 80's Phase 2. Acquire/release boundary, wait/retry semantics, staleness-window re-check, and `bootstrap_batch`'s single-acquisition-per-Map wiring all still open design questions, not defaults to copy from the identity-refresh lease pattern.
