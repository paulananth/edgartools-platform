...
7. [Investigate Apple Form 4 immutable-object content conflict](issues/87-immutable-object-conflict-apple-form4.md) — resolved: root cause confirmed via direct SEC comparison (a stable, one-byte trailing-newline drift on SEC's own served content, not a bug in this repo's byte-preserving capture path, not systemic). Decided to isolate to the one accession rather than abort the whole CIK resync -- implemented via the existing `_is_immutable_object_conflict` classification (already used by the daily-artifact-resume path), narrowly scoped so any other error type still fails the run. 2 new unit tests, full suite green (1751 passed). Not yet deployed to prod or live-verified.
