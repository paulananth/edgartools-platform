...
- [reduce-identity-refresh OOM on silver merge](issues/83-reduce-identity-refresh-oom-on-merge.md) — open, task: found live while chasing ticket 77/78's live-measurement run — `daily-incremental-ticket77-livemeasure-1785794199` OOM-killed (exit 137) in `ReduceIdentityRefresh` on `edgartools-prod-medium` (4096MB), mid-merge of `sec_company_filing`, before ever reaching `RunWarehouseTask`'s artifact-fetch stage. Same `merge_candidate_into_canonical` path as ticket 79, but a genuine-changes run (not a no-op ticket 79 would have skipped). Blocks ticket 77/78's live measurement (again) and ticket 82/49/61/63's shared "fresh execution past the deploy" evidence gate.
