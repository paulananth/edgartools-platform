...
- [Run `warehouse.gold_standalone` Medium Canaries](issues/29-run-warehouse-gold-standalone-medium-canaries.md) — The isolated current-image large-to-medium cohort passed exact five-table/470,101-row output parity, cross-run idempotency, full-window no-overlap, p95 duration (`0.45%` faster), and estimated compute-cost (`48.69%` lower) performance gates. Qualification remains blocked on matched Snowflake input-envelope identity and exercised recovery evidence. Keep `warehouse-large` operational; do not update production references or start the bake window.
- [Add Per-Run Binding Columns to MDM Tables](issues/30-add-per-run-binding-columns-to-mdm-tables.md) — Nullable `run_id` now binds entity-change and relationship-version evidence to its originating MDM operation across Postgres and the Snowflake mirror; historical/current-state rows remain untouched, relationship origin is immutable, and all evidence-producing orchestration paths share one identity.
...
Tickets 29 and 31-32 remain real, already-sharp open/claimed child tickets
(a blocked gold-standalone qualification, staged-transaction deploy support,
and a rehearsed Code Rollback cohort), not fog — pick them up directly
