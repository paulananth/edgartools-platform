...
- Wayfinder [Keep, drop, or rewrite the three-day CloudWatch retention requirement?](issues/01-decide-cloudwatch-retention-vs-seven-day-floor.md) is resolved: drop CW-01, keep seven days. Remaining open grilling: gold completeness, in-flight identity skip, ADR 0004 vs sibling (spec already synthesized defaults).
- After implementation: `/to-tickets` should slice tracer bullets as (1) architecture test + lifecycle HCL, (2) targeted apply runbook, (3) reclaim tool + fixtures, (4) operator dry-run then apply against shards / identity / gold in that order.
