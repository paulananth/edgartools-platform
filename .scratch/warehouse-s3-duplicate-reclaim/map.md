...
- [What is the latest complete gold run we must keep?](issues/02-define-latest-complete-gold-run.md) — Keep the newest complete `run_id` (current parquet for every gold table in the listing, ranked by LastModified). Partial newer runs are reclaimable. If none is complete, fall back to the union of per-table newest run_ids.
