...
- [Decide and run backlog cleanup](issues/04-decide-and-run-backlog-cleanup.md) — Live-verified (full backlog, not the 5-sample) that all 140,907 groups are fully byte-identical duplicates, including the 3,960 with a quarantined row. Mechanism: `supersede_relationship_version` only (never `close_relationship_version`/`valid_to_date` — `is_active` never flips for this table, corrected from the ticket's own "keep the earliest instance_id" premise since `instance_id` is a non-ordered UUIDv4). New dedicated write-capable module/CLI, bounded first pass before the full run, no forced graph resync. Graduates into [Ticket 05](issues/05-implement-and-run-manages-fund-duplicate-backfill.md).
- [Implement and run manages-fund-duplicate backfill](issues/05-implement-and-run-manages-fund-duplicate-backfill.md) — Built `mdm backfill-manages-fund-duplicates` (new `manages_fund_duplicate_backfill.py` + CLI subcommand), mirroring `relationship_quarantine_backfill.py`'s CLI/pagination shape without its conflict-resolution machinery. Passed pre-code `/gof-refactor-reviewer` and post-diff 3-axis `/code-review` with zero blocking findings. Implementation, unit tests (14/14), and full-suite regression are done and committed. Live-prod verified in two bounded steps (dry-run --limit 20, then a real --limit 20 run): 20 groups/60 rows superseded exactly as predicted, backlog dropped 140,907 -> 140,887, zero anomalies. Only the full unbounded backlog run (remaining ~140,887 groups) is still deliberately unattempted, pending explicit go-ahead.
