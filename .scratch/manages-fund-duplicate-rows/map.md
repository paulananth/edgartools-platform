...
- [Implement and run manages-fund-duplicate backfill](issues/05-implement-and-run-manages-fund-duplicate-backfill.md) — Built `mdm backfill-manages-fund-duplicates` (new `manages_fund_duplicate_backfill.py` + CLI subcommand), mirroring `relationship_quarantine_backfill.py`'s CLI/pagination shape without its conflict-resolution machinery. Passed pre-code `/gof-refactor-reviewer` and post-diff 3-axis `/code-review` with zero blocking findings. Implementation, unit tests (14/14), and full-suite regression are done and committed; the three live-prod verification acceptance boxes (dry-run, small real run, full backlog run against real Postgres) remain deliberately unattempted pending explicit go-ahead for production writes.
