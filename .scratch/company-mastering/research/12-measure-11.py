"""Proving Run for rule 2026-09-25.11 (company mastering ticket 12).

Version .11 is .10 plus option A (operator, 2026-09-25): the REIT code 6798
joins step 6 (a finance code with only an Emerging growth category waits),
and a new step 6b holds back an `operating` REIT with no catalog ticker.
Step ids are kept stable (the Company steps stay 8 and 10). Step 4 and 6b
name Company as their Probable Kind (step 4 measured 91 of 100 in .10).
The .10 draws designed it, so every record drawn for any earlier version is
left out of the adversarial arms here.

Everything below is drawn after .11 was frozen, on a fresh seed:

- `12-11-sample.jsonl`: 300 records per Company step, simple random;
- `12-11-adversarial.jsonl`: arms chosen by evidence the deciding step does
  ...
  read tickers only through steps 4 and 6b. So the arms are private
  offerings (Form D), Form 10 registrations, investment or real-estate
  industry codes, a REIT code with a catalog ticker and no annual report
  under step 8, no annual report under step 10, and no catalog ticker;
- `12-11-held.jsonl`: up to 100 records each that steps 6 and 6b hold back,
  read to check their Probable Kinds. They cannot be wrong.
...
are hand-read (`12-11-label.py`). Zero SEC requests.

    uv run python .scratch/company-mastering/research/12-measure-11.py \\
...
def draw(summary: Path, tickers_path: Path, out: Path) -> None:
    ...
    write(out / "12-11-sample.jsonl", sample)
    ...
    write(out / "12-11-held.jsonl", held_rows)
    (out / "12-11-population.json").write_text(
        json.dumps(
            {
...
def load(out: Path, name: str) -> list[dict]:
    return [json.loads(line) for line in (out / f"12-11-{name}.jsonl").open()]
...
def score(out: Path) -> None:
    ...
    files = {
        ...
        for name in (
            *(f"12-11-{n}.jsonl" for n in NAMES),
            "12-11-population.json",
            "12-classify.py",
            "12-measure-11.py",
            "12-11-label.py",
        )
    ...
    (out / "12-11-summary.json").write_text(
