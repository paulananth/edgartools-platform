"""Proving Run for the Account hold-back, rule 2026-09-25.13 (ticket 12).

The Account hold-back is the Forms hold-back (2026-09-25.12) plus ACCOUNT
among the fund-name words (step 5): an insurance company's separate account
is a Fund (operator, 2026-09-25), and the TIAA Real Estate Account was the
Forms hold-back's one violation. The Forms hold-back's draws designed it, so
every record drawn for any earlier version is left out of the adversarial
arms.

Everything below is drawn after the rule was frozen, on a fresh seed:

- `12-13-sample.jsonl`: 300 records per Company step, simple random;
- `12-13-adversarial.jsonl`: arms chosen by evidence the deciding step does
  ...
  investment-company filings (N-2, 497, 40-APP, N-8A), no proxy statement,
  a public REIT offering (S-11), an asset-backed issuer report and a 13F
  holdings report, and no ticker under step 8; Form D under step 10;
- `12-13-held.jsonl`: every record step 5 (fund-name words) holds back,
  read to check its Probable Kind. They cannot be wrong.
...
are hand-read (`12-13-label.py`). Zero SEC requests.

    uv run python .scratch/company-mastering/research/12-measure-13.py \\
...
def arm_of(step: str, r: dict, tickers: list[str]) -> list[str]:
    ...
    if step == "8":
        ...
        if not forms & {"DEF 14A", "DEFA14A", "DEF 14C"}:
            arms.append("step 8 with no proxy statement")
        if forms & {"S-11", "S-11/A"}:
            arms.append("step 8 with a public REIT offering (S-11)")
        if forms & {"ABS-15G", "ABS-15G/A", "10-D"}:
            arms.append("step 8 with an asset-backed issuer report")
        if forms & {"13F-HR", "13F-HR/A"}:
            arms.append("step 8 with a 13F holdings report")
...
def draw(summary: Path, tickers_path: Path, out: Path) -> None:
    ...
    write(out / "12-13-sample.jsonl", sample)
    ...
    write(out / "12-13-held.jsonl", held_rows)
    (out / "12-13-population.json").write_text(
        json.dumps(
            {
...
def load(out: Path, name: str) -> list[dict]:
    return [json.loads(line) for line in (out / f"12-13-{name}.jsonl").open()]
...
def score(out: Path) -> None:
    ...
    files = {
        ...
        for name in (
            *(f"12-13-{n}.jsonl" for n in NAMES),
            "12-13-population.json",
            "12-classify.py",
            "12-measure-13.py",
            "12-13-label.py",
        )
    ...
    (out / "12-13-summary.json").write_text(
