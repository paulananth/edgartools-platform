...
- [04 — Thin Landing-Zone Backfill Rows Null Out Every Non-Coalesced Column](issues/04-thin-backfill-nulls-other-columns.md) — `update_accounting_flag_scores` now records the complete post-update row (`RETURNING *`) instead of a thin 5-column one, stopping new corruption at the source rather than widening the dbt collapse's coalesce list. Schema-agnostic enough to pick up Ticket 33's validity-interval trio automatically. Repair of already-corrupted historical rows split to Ticket 05. Not yet deployed.
