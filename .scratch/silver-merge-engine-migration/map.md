...
- [Retire the two remaining local-store couplings](issues/16-retire-remaining-local-store-couplings.md) — resolved 2026-09-14 (operator decisions). The `compute-windows` reference snapshot is an empty 2.3 MB file: drop the upload and the manifest field (corrected 2026-09-15: `reduce-identity-refresh` stays — it is still `daily_incremental`'s batch-completeness gate). `parse-ownership-bronze` has had no real run in 90 days and reads the never-hydrated store: retire it. Both deletions folded into Ticket 15.
- [Move the write path into a store-free module](issues/14-move-write-path-off-silver-database.md) — resolved in code 2026-09-14. `SilverLandingStore` (new module) holds the passthrough, in-run lookup and every writer, moved verbatim; `SilverDatabase` subclasses it and keeps only DuckDB until Ticket 17. No call site moved. `get_table_counts` on the local store deleted; the run summary counts bookkeeping's tables only. Frontier is now Ticket 15.
