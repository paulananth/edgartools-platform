...
- [`sec_company_ticker`'s DuckDB and Snowflake copies disagree](../duckdb-retirement-cutover/issues/19-sec-company-ticker-cross-store-divergence.md) (duckdb-retirement-cutover Ticket 19, blocks Ticket 21 and so Ticket 09) — root-caused 2026-09-14, closure pending operator decision. Snowflake is right: the old canonical DuckDB file kept 512 tickers SEC had dropped, because the publish merge never deleted canonical-only rows; the orphan count is the catalog-vs-captured-companies contract, not missing data. Proposed: close as explained and unblock Ticket 21 (which still needs its own go-ahead).
