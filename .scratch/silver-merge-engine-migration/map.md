...
- [Retire the two remaining local-store couplings](issues/16-retire-remaining-local-store-couplings.md) — resolved 2026-09-14 (operator decisions). The `compute-windows` reference snapshot is an empty 2.3 MB file with no reader left: drop the upload, the manifest field and the dead `reduce-identity-refresh` command. `parse-ownership-bronze` has had no real run in 90 days and reads the never-hydrated store: retire it. Both deletions folded into Ticket 15.
