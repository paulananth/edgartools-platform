...
- [_run_grouped_concurrent single end-of-group commit](issues/04-run-grouped-concurrent-single-end-of-group-commit.md) — claimed, not yet resolved: found live during Ticket 03's own verification run -- one oversized security-title group ran invisibly (no committed writes) for 23+ minutes and lost all progress when the run was interrupted, because `_process_group` only committed once at the end of its whole group's loop, not periodically. Fix implemented and unit-tested; still being verified against the real oversized-group data that surfaced this before this ticket resolves and this line moves to Decisions so far.
