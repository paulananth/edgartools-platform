...
**`SILVER_LANDING_EXPORT_ROOT` flipped on — 2026-08-13.** PR #412 added the env var to the warehouse task definition's `environment_values` (value: `s3://edgartools-prod-snowflake-export-690839588395/warehouse/artifacts/silver_landing`, the same prefix the storage integration/IAM widen and `LOAD_SILVER_LANDING_TASK` were already configured for); `deploy-aws-application.sh --env prod --skip-build --enable-mdm` redeployed all 6 ECS task definitions and 18 Step Functions state machines against the live-confirmed warehouse (`sha256:55cffc18...`) and MDM (`sha256:ac245df9...`) image digests. Along the way, found the deployment manifest (`infra/aws-prod-application.json`) had a stale warehouse digest (`sha256:64ff30ae...`, from an Aug-12 write, superseded by an Aug-13 redeploy that updated the live task definitions without updating the manifest) — worked around by passing the live-confirmed digest explicitly via `--image-ref` rather than trusting the manifest default; the redeploy's own manifest rewrite closed that staleness gap. Verified live via `aws ecs describe-task-definition` that the env var landed with the correct value. Real bronze-capture runs now populate the silver-landing zone — this was the last deferred piece of Ticket 07.
