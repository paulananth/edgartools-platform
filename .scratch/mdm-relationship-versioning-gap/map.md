...
- [Ticket 11: AUDITED_BY closing lacks chronological guard](issues/11-audited-by-closing-lacks-chronological-guard.md) — confirmed live first (`/diagnosing-bugs`): `AUDITED_BY` has zero rows in prod, so no actual corruption occurred yet, but the gap is real and would fire on the first out-of-order/restated row. Investigated the ticket's own "route through the shared `_deactivate_if_properties_changed` helper" suggestion and found it's not viable — that helper is keyed by `(source, target)` and compares properties for the SAME pair, but an auditor change means the TARGET itself changes, which the shared helper's keying can't express. Added the missing `confirmed_chronologically_after` guard directly to `_derive_audited_by`'s own inline query instead. Reproduced live with a real regression test: the query's own `ORDER BY audited_period_end` sorts a late-filed restatement for an EARLIER fiscal year BEFORE a LATER year's original filing, reachable within a single batch — before the fix this crashed with `IntegrityError` (`ck_rel_instance_valid_interval`), not silent corruption. 1 new test, full repo suite green. Deployed and run against real prod 2026-09-10/11 (`mdm derive-relationships --relationship-type AUDITED_BY`, exit 0) — the new guard doesn't crash, but couldn't be exercised: `sec_auditor_report_evidence`/`sec_accounting_flag` are both empty in prod (separate, pre-existing, unaddressed gap tracked in `mdm-relationship-incremental-filters` Ticket 03, not this map).
- [Ticket 12: verify Ticket 07's fix was live before Tickets 09/10's rebuilds](issues/12-verify-ticket07-fix-live-before-post-backfill-rebuilds.md) — graduated from this map's own fog and resolved in the same session. Verified directly via ECR push timestamps and ECS task-definition revision history (not inferred from git alone): the image deployed at `edgartools-prod-mdm-large:194` (live 2026-09-10T06:17 ET) already contained Ticket 07's quarantine-filter fix, and that revision was live before both Ticket 09's and Ticket 10's post-backfill graph rebuilds ran — so their "exact parity confirmed" results are real, not the false-clean signal Ticket 04 warned about. Also confirmed the second half of the original fog question: a post-backfill graph rebuild is **not automatic** — both tickets triggered it as an explicit manual step; nothing in `relationship_quarantine_backfill.py`'s CLI wires this, a real (undemonstrated-in-practice) operational gap noted as new fog below.
...
- ~~Whether the ~199K already-quarantined rows' duplicate graph edges
  ...
  ordinary `sync-graph` run's full generation rebuild self-heals~~ --
  resolved by [Ticket 12](issues/12-verify-ticket07-fix-live-before-post-backfill-rebuilds.md):
  no targeted correction needed, the manual rebuilds Tickets 09/10 already
  ran were sufficient and are confirmed (not assumed) to have worked.
- Should `relationship_quarantine_backfill.py`'s CLI automatically trigger
  a `sync-graph` rebuild afterward, instead of relying on the operator to
  remember the separate manual step every time (confirmed via Ticket 12:
  today it's 100% manual, both real runs so far got it right, but nothing
  enforces or alerts if a future run doesn't)? Not yet a ticket -- no
  evidence anyone has actually forgotten it in practice, so there's no
  live cost to point at yet, just a latent one.
