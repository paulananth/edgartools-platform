...
- [x] A new scheduled job (not event-triggered, per Ticket 09's Answer)
...
- [x] A separate daily rollup derives `business_date`-level
...
- [x] The two SQL gaps CLAUDE.md already documents in the existing
...
- [x] A watchdog SLO (reusing `publication.py`'s 5-minute-warning/
...
- [x] No new retry/DLQ/repair mechanism is built on the aggregator itself
...
`edgar_warehouse/serving/watermark_aggregator.py` is observe-only.
`reconcile_cause_reference` injects four stage readers (silver, mdm, gold,
graph), writes one `CauseAlignment` row, and names `stuck_stage` from
silver → mdm → gold → graph. `rollup_business_date` calls
`evaluate_agent_grade` only after every cause on that date is aligned.
`compute_alignment_freshness` reuses `publication.py`'s 5-minute warning /
15-minute hard-alert and reports which stage is stuck.

There is no aggregator repair API. A test flips the silver reader from
incomplete to complete; the next pass aligns and the SLO returns to
normal.

Durable rows: `JsonAlignmentStore` (`--state-file`) so a scheduled CLI
pass keeps `first_seen_at`. CLI:
`edgar-warehouse reconcile-decision-watermark --business-date … --cause-reference …`
(repeatable). Completeness flags stand in until production wires the
Ticket 35/36/39/40 readers.

SQL sketches:
- `01_subject_feature_screen.sql` universe is `MDM_COMPANY_ENTITY` where
  `tracking_status='active'`, not warehouse `COMPANY`.
- `02_subject_bundle_read_issuer.sql` `BUNDLE_AUDITOR` reads
  `EDGARTOOLS_SOURCE.SEC_AUDITOR_REPORT_EVIDENCE` (`principal_firm_name` /
  `pcaob_firm_id`).

Tests: `tests/serving/test_watermark_aggregator.py`.
