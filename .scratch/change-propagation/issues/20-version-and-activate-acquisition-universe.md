...
- [~] The Source Family Registry versions logical keys, acquisition mode,
  ...
  producers as executable policy data. **Partial** — see Answer below.
- [~] Adding coverage creates a scoped baseline and catch-up obligation;
  ...
  retiring existing SEC facts. **Partial** — catch-up obligation is fully
  done; the removal boundary is not enforced. See Answer below.
...
Bullets 1 and 2 are **only partially done** — `/code-review`'s Spec pass
found real gaps, not nitpicks: `in_scope_forms` is genuinely wired into
`drive_filing_discovery.py`'s scope filter, but `acquisition_mode`/
`completeness_policy`/`discovery_policy`/`required_producers` are captured
and persisted with no consumer anywhere in the codebase (inert audit
metadata, not executable policy), and `coverage_end_date` is stored but
never compared against any date — a `'remove'`d family is excluded
immediately on activation, not "at an explicit boundary." A third gap
surfaced alongside these: no committed bootstrap path exists to open and
activate the first registry version in prod, so applying migration 014 and
redeploying would make `drive-filing-discovery-for-date`/
`capture-filing-artifact` fail closed in production today. All three are
real, scoped decisions rather than obvious bug fixes, so rather than guess
at the right behavior under review pressure, they're filed as
...
**Status:** ready-for-agent (remains open — reopen fully resolved once 32
