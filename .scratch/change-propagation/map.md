...
- [29 — Deploy the gated acquisition path to prod and dry-run it](issues/29-deploy-and-dry-run-gated-acquisition-path.md) — **Open, partially done.** Migration 013 applied to prod (two genuine provisioning bugs found/fixed, PR #453) and current images deployed, but the live dry run itself is blocked by newly-discovered bugs in the general silver merge/publish path — see 31. Now blocked by 31 (and see 30, non-blocking).
- [30 — Fence `application` from acquisition ledger tables under Snowflake Postgres's `snowflake_write` role](issues/30-fence-application-from-acquisition-tables-under-snowflake-write.md) — Surfaced while resolving Ticket 29: `application` ambiently inherits full DML on the fenced acquisition tables via Snowflake Postgres's managed `snowflake_write` role, bypassing migration 013's explicit `REVOKE`s. Does not block Ticket 29's dry run.
- [31 — `EXCLUDED_OPERATIONAL_TABLES` content never reaches canonical silver once canonical exists](issues/31-excluded-operational-tables-never-reach-canonical-silver.md) — Surfaced while resolving Ticket 29: `merge_candidate_into_canonical` never copies `EXCLUDED_OPERATIONAL_TABLES` tables into canonical once canonical exists, contradicting its own documented intent; blocks Ticket 29's dry run.
