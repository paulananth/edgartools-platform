...
ordering (renamed to Publish-before-"Publish Relationships" by
mdm-stage-renaming ticket 01), not the whole tail's flags/Catch/retry shape
-- those genuinely differ per MDM Pipeline Machine (see the ticket's
addendum) and stay caller-owned. This module wires already-built
Publish/"Publish Relationships"/Reconcile state dicts into the correct,
data-architecture-mandated order (docs/data-architecture.md Issue 3) and
optionally appends GoldRefresh -- nothing more. Deploy-aws-application.sh's
Python heredocs import this via sys.path.insert(0, SCRIPT_DIR) so every MDM
Pipeline Machine shares the exact same ordering logic instead of
hand-typing Next pointers.
"""
