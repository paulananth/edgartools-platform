...
cat infra/snowflake/sql/bootstrap/18_silver_loader_read_grants.sql; } | snow sql --connection ${SNOW_CONNECTION} -i"
  ...
  add_stage \
    ...
    "Starts the bronze_seed_silver_gold Step Function, which discovers CIKs directly from existing S3 bronze (zero new SEC calls) and chains SeedFromBronze -> BatchSilver -> Mastering -> MdmBackfill -> Publish Relationships -> Reconcile -> GoldRefresh on ECS. Polls until the execution reaches a terminal state and fails this stage unless it reaches SUCCEEDED. This is the canonical one-click path for cold-starting or recovering an environment's silver/MDM/gold from a bronze snapshot that's already in S3 (e.g. synced in from another environment) -- see CLAUDE.md's phased pipeline table. The input MUST include \"release_mode\": false explicitly (confirmed live 2026-08-08 against a brand-new account) -- the deployed state machine's entry state, ReleaseModeCheck, is a Choice state testing \$.release_mode, and AWS Step Functions throws a hard States.Runtime error (not a graceful fall-through to Default) when a Choice state's Variable path is entirely absent from the input, not just when it fails to match." \
...
while true; do
  ...
  case \"\${status}\" in
    ...
    FAILED|ABORTED|TIMED_OUT)
      ...
      echo \"If only BatchSilver succeeded and a later stage (Mastering etc.) failed, and you intend to redeploy a fix before retrying: do NOT redrive this execution -- AWS Step Functions pins a redriven execution to the task-definition revision that was active when it last reached that state, not whatever you redeploy afterward. Start a fresh execution instead so it binds to the current state machine definition.\" >&2
      exit 1
      ;;
    *) sleep 60 ;;
  esac
