...
bash infra/scripts/bootstrap-prod-mdm.sh --env-name ${ENVIRONMENT} --snow-connection ${SNOW_CONNECTION} --instance-name ${mdm_instance_name_q} --aws-profile ${AWS_PROFILE_NAME} --aws-region ${AWS_REGION_NAME} --name-prefix edgartools-${ENVIRONMENT}"
  ...
  add_stage \
    ...
    "Creates the 19 MDM Postgres-mirror tables in ${db_name}.MDM (09_mdm_mirror_schema.sql) and the NEO4J_GRAPH_MIGRATION graph destination schema plus its CREATE-SCHEMA-ON-DATABASE grant (10_graph_schema.sql, then re-applies the Neo4j Native App grants against that same schema) -- all three previously created only by an uncommitted manual shell session during the original go-live cutover, so a fresh account rebuild silently came back with an empty MDM schema and no graph schema at all (CLAUDE.md's 'MDM Snowflake mirror schema lost on cutover' incident; reproduced live 2026-08-22 while writing this stage -- ${db_name}.MDM had zero tables and ${db_name}.NEO4J_GRAPH_MIGRATION did not exist at all, both blocking a live bronze_seed_silver_gold execution's MdmExport/MdmSync steps until fixed by hand). Must run after 'Snowflake Postgres / graph prerequisites' above, which creates the MDM schema itself but not its tables. Running the Native App grants here too (ahead of the 'MDM + graph: connectivity...' stage's own, still-idempotent re-application further below) corrects that later stage's own comment, which assumed 'mdm sync-graph' creates NEO4J_GRAPH_MIGRATION on its own -- it can't: CREATE SCHEMA IF NOT EXISTS evaluates the CREATE SCHEMA privilege before checking existence, so EDGARTOOLS_${ENV_UPPER}_LOADER needs that grant (applied here) before it can create the schema at all, not just before its later grants can be re-applied." \
...
cat infra/snowflake/sql/bootstrap/18_silver_loader_read_grants.sql; } | snow sql --connection ${SNOW_CONNECTION} -i"
  ...
  add_stage \
    ...
    "Starts the bronze_seed_silver_gold Step Function, which discovers CIKs directly from existing S3 bronze (zero new SEC calls) and chains SeedFromBronze -> BatchSilver -> MdmRun -> MdmBackfill -> MdmSync -> MdmVerify -> GoldRefresh on ECS. Polls until the execution reaches a terminal state and fails this stage unless it reaches SUCCEEDED. This is the canonical one-click path for cold-starting or recovering an environment's silver/MDM/gold from a bronze snapshot that's already in S3 (e.g. synced in from another environment) -- see CLAUDE.md's phased pipeline table. The input MUST include \"release_mode\": false explicitly (confirmed live 2026-08-08 against a brand-new account) -- the deployed state machine's entry state, ReleaseModeCheck, is a Choice state testing \$.release_mode, and AWS Step Functions throws a hard States.Runtime error (not a graceful fall-through to Default) when a Choice state's Variable path is entirely absent from the input, not just when it fails to match." \
...
execution_arn=\"\$(aws --profile ${deployer_q} --region ${AWS_REGION_NAME} stepfunctions start-execution --state-machine-arn \"\${state_machine_arn}\" --name \"\${execution_name}\" --input '{\"batch_size\": 100, \"release_mode\": false}' --query executionArn --output text)\"
