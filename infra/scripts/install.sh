...
cat infra/snowflake/sql/bootstrap/18_silver_loader_read_grants.sql; } | snow sql --connection ${SNOW_CONNECTION} -i"
  ...
  add_stage \
    ...
    "Starts the one_click_data_refresh Step Function, which discovers CIKs directly from existing S3 bronze (zero new SEC calls) and chains Initialize From Bronze -> Clean and Merge Filings -> Mastering -> Infer Relationships -> Publish Relationships -> Reconcile -> Publish Business Data on ECS. Polls until the execution reaches a terminal state and fails this stage unless it reaches SUCCEEDED. This is the canonical one-click path for cold-starting or recovering an environment's silver/MDM/gold from a bronze snapshot that's already in S3 (e.g. synced in from another environment) -- see CLAUDE.md's phased pipeline table. The input MUST include \"release_mode\": false explicitly (confirmed live 2026-08-08 against a brand-new account) -- the deployed state machine's entry state, ReleaseModeCheck, is a Choice state testing \$.release_mode, and AWS Step Functions throws a hard States.Runtime error (not a graceful fall-through to Default) when a Choice state's Variable path is entirely absent from the input, not just when it fails to match." \
