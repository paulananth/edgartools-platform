...
echo \"This runs BatchSilver sequentially (maxConcurrency=1, required while no shard manifest exists yet). With PR95's merge_filings bulk-upsert optimization in the deployed warehouse image, cached-bronze batches should be materially faster than the old per-row path. If progress looks like the old multi-minute-per-batch behavior, stop and verify the active task definition image was freshly published from a commit that contains PR95.\"
last_succeeded=-1
while true; do
