...
if [[ ${RUN_DBT} -eq 1 ]]; then
  ...
  # Terraform-managed "deployer" role (EDGARTOOLS_PROD_DEPLOYER in prod), NOT the EDGARTOOLS_LOADER
  # role that now owns the EDGARTOOLS_GOLD dynamic tables (infra/snowflake/sql/bootstrap/08_loader_role.sql).
  # Terraform has no knowledge of EDGARTOOLS_LOADER yet. Running --run-dbt as-is will re-flip
  ...
  # this is reconciled in Terraform (either make EDGARTOOLS_LOADER the deployer role, or teach this
