...
--   DIMENSIONAL — EARNINGS_RELEASE source table carries FK surrogate keys
--   (fact_key, company_key, filing_date_key, period_end_date_key, form_key)
--   generated by _build_fact_earnings_release(), alongside the natural-key
--   columns (cik, accession_number, filing_date, period_end) for direct join
--   to silver-tracked entities.
--
...
with base as (
    select * from {{ source("edgartools_source", "EARNINGS_RELEASE") }}
),
