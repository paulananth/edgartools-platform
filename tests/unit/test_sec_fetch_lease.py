...
def test_acquire_sec_fetch_lease_end_to_end(tmp_path) -> None:
    """Full _execute_warehouse_bronze_capture path (not just the
    _capture_bronze_raw block covered above): the lease lives in the
    bookkeeping store and the result lands as lease_result.json under the
    bronze root, where the Step Functions Choice state reads it."""
    from edgar_warehouse.infrastructure.dataset_path_catalog import default_path_resolver
...
def test_release_sec_fetch_lease_end_to_end(tmp_path) -> None:
    context = _context(tmp_path)
