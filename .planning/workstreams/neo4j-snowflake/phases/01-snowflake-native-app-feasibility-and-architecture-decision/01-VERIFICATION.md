...
| 2 | A recorded architecture decision states that Snowflake-hosted Neo4j replaces external Neo4j for this milestone; no dual external validation path is required. | VERIFIED | `01-ARCHITECTURE-DECISION.md` has `Accepted for milestone planning`, states the Native App replaces external Neo4j, says there is no external Neo4j parallel validation target, and rejects external Aura/Bolt, self-hosted Neo4j, dual-write validation, Azure Container Apps revival, and dashboard-owned graph writes. |
...
| ISO-02 | 01-01, 01-02, 01-03 | Changes remain AWS/Snowflake-focused and do not introduce non-AWS deployment paths or secret-management paths. | SATISFIED | Runbook and ADR reject Azure/container-app revival and external `NEO4J_*` milestone validation; no source, Terraform, generated JSON, or sibling workstream files changed. |
