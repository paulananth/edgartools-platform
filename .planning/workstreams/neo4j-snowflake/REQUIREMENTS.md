...
- [ ] **VERIFY-01**: Verification reports matching node counts between MDM active entities, Snowflake graph node tables/views, and Native App graph projection results.
- [ ] **VERIFY-02**: Verification reports exact relationship parity between active MDM relationship rows and Snowflake graph edge tables/views by relationship type.
- [ ] **VERIFY-03**: Verification reports query-level graph traversal checks that prove important ownership/adviser/fund relationships are reachable.
- [ ] **VERIFY-04**: Verification includes dashboard comparison against the Snowflake-hosted graph target.
- [ ] **VERIFY-05**: Verification includes an end-to-end AWS pipeline run that reaches the Snowflake-hosted graph validation path.
