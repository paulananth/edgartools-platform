...
- [ ] **EDGE-09**: `EMPLOYED_BY` (person→company) — **source artifact: DEF 14A proxy filing documents** (feed `sec_executive_record`) **plus Item 5.02 8-K employment events** (feed `sec_employment_event`). **Root-caused 2026-07-13 (Phase 6, 06-06): ROOT-CAUSED / FIX DEFERRED, not populated.** Original finding: `_is_configured_parser_form` never selected DEF14A/DEFA14A/8-K for the bulk artifact-fetch pipeline. **Update 2026-07-26: that fetch-gate fix has since shipped and run at scale** — `_is_configured_parser_form` (`edgar_warehouse/application/warehouse_orchestrator.py`) now includes `DEF 14A`, `DEF 14A/A`, `DEFA14A`, `PRE 14A`, and Item 5.02 8-Ks; Snowflake SOURCE confirms `EXECUTIVE_RECORD` has 13,403 live rows. **New gap found 2026-07-26** while root-causing the identical EDGE-11 symptom: `ShardedSilverReader._TABLES` (`edgar_warehouse/silver_support/sharded_reader.py`) never registered `sec_employment_event` as a cross-shard view (added in the same commit as its sibling tables, d20cad8, but omitted from the allowlist) — so any MDM derive query joining it hits a swallowed "missing_source_table" skip. Fixed same commit as EDGE-11's fix (see below); `sec_executive_record` was already registered, so the DEF 14A half of EMPLOYED_BY was not affected by this bug — whether it's populated is unverified as of this note (out of scope of the reader-registration fix). See `06-PHASE-CLOSURE-LEDGER.md` for the original finding.
...
- [ ] **EDGE-11**: `INSTITUTIONAL_HOLDS` (adviser→security) — **source artifact: 13F-HR INFORMATION TABLE XML documents** (feed `sec_thirteenf_holding`). **Root-caused 2026-07-13 (Phase 6, 06-04/06-06): ROOT-CAUSED / FIX DEFERRED.** Original finding shared its root cause with EDGE-09 (`_is_configured_parser_form` never selects 13F-HR for bulk artifact fetch). **That finding is stale as of 2026-07-26** — the fetch-gate fix shipped and ran at scale: `EDGARTOOLS_PROD.EDGARTOOLS_SOURCE.SEC_THIRTEENF_HOLDING` has 6.8M live rows. **Real, currently-confirmed root cause (2026-07-26):** MDM's relationship-derive step reads silver through `ShardedSilverReader` (`edgar_warehouse/silver_support/sharded_reader.py`), whose hardcoded `_TABLES` allowlist governs which tables get cross-shard UNION ALL views. `sec_thirteenf_filing` (INSTITUTIONAL_HOLDS's JOIN partner, holding filing-level dedup/amendment metadata) was added to the schema in the same commit as `sec_thirteenf_holding` (d20cad8) but never added to `_TABLES` — so the derive query's JOIN raises a DuckDB catalog error that `_find_missing_source_table`'s deliberately-broad matching swallows as a legitimate "not loaded yet" skip (confirmed via live CloudWatch: `mdm-mdm-large/edgar-warehouse/7fd06878e8254bcab9cbdb4263066ab8`, 2026-07-25T23:26:52Z, `reason: "missing_source_table", "source_table": "sec_thirteenf_filing"`). **Fixed 2026-07-26:** `sec_thirteenf_filing` (and sibling `sec_employment_event`, see EDGE-09) added to `_TABLES`; regression test `tests/unit/test_sharding.py::test_sharded_silver_reader_exposes_thirteenf_filing_and_employment_event` reproduces the exact prod failure mode and passes with the fix. No new SEC fetching required — source data already exists in the shards. Full evidence chain: `.scratch/release-readiness/issues/06-define-full-chain-launch-gate.md`. **Not marked Complete** — fix is written but not yet deployed to prod / re-derived / graph-verified.
