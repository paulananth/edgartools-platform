...
The warehouse-side sibling of MDM Pipeline Machine: a Step Functions state machine chaining bronze+silver capture into the MDM/gold tail, built from one shared template (`build_workflow_states`) with a `daily_incremental`-specific branch (identity-refresh lease + windowed identity refresh ahead of capture). 2 machines: `bootstrap`, `daily_incremental`.
_Avoid_: MDM Pipeline Machine (a distinct family whose head varies per machine, not one shared template); Single-Command Workflow Machine (one task, not a capture→MDM→gold chain); Load History Machine (a bespoke, more elaborately staged version of the same idea, not built from this shared template)
