...
**MDM Utility Machine**:
A Step Functions state machine wrapping exactly one MDM CLI subcommand (`mdm run`, `backfill-relationships`, `sync-graph`, `verify-graph`, `counts`, `migrate`, `check-connectivity`), generated by the single shared `write_mdm_workflow_definition` function with only its command expression and task-definition ARN varying per machine. Exactly 7 machines fit this definition today.
