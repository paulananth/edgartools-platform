...
The warehouse-side sibling of MDM Utility Machine: a Step Functions state machine wrapping exactly one warehouse CLI subcommand in a single ECS task (optionally wrapped in the `sec_fetch_active` lease for the sole one that still calls SEC at meaningful volume: `targeted_resync`), generated by the shared `write_single_workflow_definition` function — a different shared function from MDM Utility Machine's. 2 machines today: `targeted_resync`, `gold_refresh`. (`bootstrap_full`, `load_daily_form_index_for_date`, and `catch_up_daily_form_index` were retired from this loop by state-machine-consolidation wayfinder map ticket 09, 2026-09-05 — their standalone wrappers had zero, or for `load_daily_form_index_for_date`, dry-run-only, executions ever; their CLI commands remain valid for direct/manual ECS invocation, just no longer wrapped in an automated Step Function. `seed_universe` left this loop even earlier, ticket 07, merged into the `seed` machine. `full_reconcile` — the SEC-drift reconciliation capability — was decommissioned entirely, not just consolidated: zero executions ever, no schedule, and no live caller of its underlying CLI command/code remained.)
_Avoid_: MDM Utility Machine (MDM subcommands, mode-dispatched through one machine, not these two); Load History Machine / Graph Generation Build Machine (bespoke, no shared builder); assuming `bootstrap_full`'s CLI command is gone because its machine is — `bootstrap-full` remains a `SOURCE_EXPORT_COMMANDS` member and a valid direct-invocation command, `command_task_profile()` still resolves its task size for that reason alone

**MDM Pipeline Machine**:
A Step Functions state machine chaining a distinct head (bronze/silver capture, ownership parsing, or nothing) into the shared Publish→"Publish Relationships"→Reconcile tail (renamed from MdmExport→MdmSync→MdmVerify by mdm-stage-renaming ticket 01), usually ending in gold-refresh. 2 today with live code and a live, non-orphaned AWS object: `one_click_data_refresh` (formerly `bronze_seed_silver_gold`, renamed 2026-09-06), `residual_holds_graph`. (`mdm_gold` and `ownership_mdm_gold` were retired outright, own tickets, predating this entry's last update; `silver_mdm_gold` retired by state-machine-consolidation wayfinder map ticket 09, 2026-09-05 — zero executions ever, deleted outright, both code and AWS object. Ticket 09 also confirmed `one_click_data_refresh`'s default path is NOT dead — install.sh's documented cold-start/recovery procedure depends on it — so only that machine's fate stays undecided pending a separate follow-up, not retired here.)
_Avoid_: Treating these as duplicated in shape beyond the tail's ordering — the flags, Catch clauses, retry counts, and (for `one_click_data_refresh`, until silver-merge-engine-migration Ticket 11 deleted it) an entire second "strict" branch are genuinely different per machine, not copy-paste variance (state-machine-consolidation wayfinder map, ticket 02 addendum); assuming "retired" always means the live AWS object is gone too — `edgartools-prod-mdm-gold` was found still live in AWS (zero executions, matching its code-retired status) while auditing this entry for ticket 09, an orphan left over from `mdm_gold`'s earlier code-only retirement; cleaned up by ticket 12, 2026-09-06 (rollback snapshot captured, zero executions ever reconfirmed fresh, deleted live)
