...
A Step Functions state machine chaining a distinct head (bronze/silver capture, ownership parsing, or nothing) into the shared Publish→"Publish Relationships"→Reconcile tail (renamed from MdmExport→MdmSync→MdmVerify by mdm-stage-renaming ticket 01), usually ending in gold-refresh. 2 today with live code and a live, non-orphaned AWS object: `bronze_seed_silver_gold`, `residual_holds_graph`. (`mdm_gold` and `ownership_mdm_gold` were retired outright, own tickets, predating this entry's last update; `silver_mdm_gold` retired by state-machine-consolidation wayfinder map ticket 09, 2026-09-05 — zero executions ever, deleted outright, both code and AWS object. Ticket 09 also confirmed `bronze_seed_silver_gold`'s default path is NOT dead — install.sh's documented cold-start/recovery procedure depends on it — so only that machine's fate stays undecided pending a separate follow-up, not retired here.)
_Avoid_: Treating these as duplicated in shape beyond the tail's ordering — the flags, Catch clauses, retry counts, and (for `bronze_seed_silver_gold`) an entire second "strict" branch are genuinely different per machine, not copy-paste variance (state-machine-consolidation wayfinder map, ticket 02 addendum); assuming "retired" always means the live AWS object is gone too — `edgartools-prod-mdm-gold` was confirmed still live in AWS (zero executions, matching its code-retired status) while auditing this entry for ticket 09, an orphan left over from `mdm_gold`'s earlier code-only retirement, not yet cleaned up as of this writing
