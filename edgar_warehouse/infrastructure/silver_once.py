...
def has_successful_ownership_parse(
    ...
    *,
    ...
    """True when the bookkeeping store has a succeeded parse_run for this
    accession at parser_version.

    Returns False (do not skip) when a probe cannot answer -- prefer network
    over a hard crash on incomplete test/adapter surfaces.

    There is no fallback to ownership rows: the durable parse run already covers
    earlier runs, and the ownership tables are landing-only (silver-merge-engine-
    migration Ticket 06c).
    """
    ...
    return bool(
        has_successful_parse_run is not None
        and has_successful_parse_run(
