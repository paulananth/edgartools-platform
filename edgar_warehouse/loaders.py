...
def seed_universe_loader(
    ...
    """Parse company_tickers_exchange.json into CIK universe rows.

    Returns one row per (cik, ticker) pair with enough metadata to seed
    sec_tracked_universe. Handles both the columnar fields/data format
    (company_tickers_exchange) and the simple dict-per-entry format
    (company_tickers).
    """
    rows: list[dict[str, Any]] = []
