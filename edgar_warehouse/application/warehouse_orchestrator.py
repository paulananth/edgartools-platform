...
def _apply_submission_snapshot_to_silver(
    *,
    ...
    # individual-filer-company-misclassification map, Ticket 03/04: whether a
    # CIK is an individual is only knowable once main_payload is fetched
    # (here), and it overrides whatever the block above computed, same as
    # _demote_deregistered_ciks' unconditional overwrite for Form 15. Once a
    # CIK is known non-company, _filter_ciks_to_universe's "active only"
    # filter excludes it from every future daily_incremental sweep.
    #
    # Corrected 2026-09-24: entityType 'other' alone is not an individual
    # (see is_individual_filer). A CIK the old rule already demoted is not
    # restored here: its status before the demotion (deregistered, paused) is
    # no longer known, so an automatic restore could re-activate a CIK that
    # should stay inactive. Restoring them is a separate, deferred repair.
    if is_individual_filer(main_payload):
