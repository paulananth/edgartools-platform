...
def _execute_drive_adv_filing_discovery_for_date(args: Any) -> int:
    # Import lazily: this command's own workflow imports the orchestrator,
    # which in turn consults this registry for scope and write planning.
    from edgar_warehouse.application.commands.drive_adv_filing_discovery_for_date import (
        execute,
    )
...
def _resolve_drive_adv_filing_discovery_for_date_scope(
    *,
    arguments: dict[str, Any],
    now: datetime,
    silver_root: StorageLocation | None,
...
def _plan_drive_adv_filing_discovery_for_date_writes(
    *,
    command_path: str,
    run_id: str,
    scope: dict[str, Any],
...
def _execute_drive_submissions_discovery(args: Any) -> int:
    # Import lazily: this command's own workflow imports the orchestrator,
    # which in turn consults this registry for scope and write planning.
    from edgar_warehouse.application.commands.drive_submissions_discovery import (
        execute,
    )
...
def _resolve_drive_submissions_discovery_scope(
    *,
    arguments: dict[str, Any],
    now: datetime,
    silver_root: StorageLocation | None,
...
def _plan_drive_submissions_discovery_writes(
    *,
    command_path: str,
    run_id: str,
    scope: dict[str, Any],
...
def _execute_drive_reference_catalog_discovery(args: Any) -> int:
    # Import lazily: this command's own workflow imports the orchestrator,
    # which in turn consults this registry for scope and write planning.
    from edgar_warehouse.application.commands.drive_reference_catalog_discovery import (
        execute,
    )
...
def _resolve_drive_reference_catalog_discovery_scope(
    *,
    arguments: dict[str, Any],
    now: datetime,
    silver_root: StorageLocation | None,
...
def _plan_drive_reference_catalog_discovery_writes(
    *,
    command_path: str,
    run_id: str,
    scope: dict[str, Any],
...
def _execute_drive_adv_bulk_dataset_discovery(args: Any) -> int:
    # Import lazily: this command's own workflow imports the orchestrator,
    # which in turn consults this registry for scope and write planning.
    from edgar_warehouse.application.commands.drive_adv_bulk_dataset_discovery import (
        execute,
    )
...
def _resolve_drive_adv_bulk_dataset_discovery_scope(
    *,
    arguments: dict[str, Any],
    now: datetime,
    silver_root: StorageLocation | None,
...
def _plan_drive_adv_bulk_dataset_discovery_writes(
    *,
    command_path: str,
    run_id: str,
    scope: dict[str, Any],
...
_ACQUISITION_COMMAND_REGISTRATIONS = build_acquisition_command_registry(
    (
        ...
        AcquisitionCommandRegistration(
            name="drive-submissions-discovery",
        ...
        AcquisitionCommandRegistration(
            name="drive-reference-catalog-discovery",
            execute=_execute_drive_reference_catalog_discovery,
            resolve_scope=_resolve_drive_reference_catalog_discovery_scope,
            planned_writes=_plan_drive_reference_catalog_discovery_writes,
        ),
        AcquisitionCommandRegistration(
            name="drive-adv-bulk-dataset-discovery",
            execute=_execute_drive_adv_bulk_dataset_discovery,
            resolve_scope=_resolve_drive_adv_bulk_dataset_discovery_scope,
            planned_writes=_plan_drive_adv_bulk_dataset_discovery_writes,
        ),
        AcquisitionCommandRegistration(
            name="drive-adv-filing-discovery-for-date",
            execute=_execute_drive_adv_filing_discovery_for_date,
            resolve_scope=_resolve_drive_adv_filing_discovery_for_date_scope,
            planned_writes=_plan_drive_adv_filing_discovery_for_date_writes,
        ),
    )
)
