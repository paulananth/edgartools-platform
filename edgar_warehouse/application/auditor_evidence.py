...
def _report_date_from_text(text: str) -> date | None:
    heading = re.search(
        r"report of independent registered public accounting firm", text, re.I
    )
    if not heading:
        return None
    match = re.search(
        r"([A-Za-z]+\s+\d{1,2},\s+\d{4})", text[heading.start():heading.start() + 30000]
    )
    if not match:
        return None
...
def parse_auditor_evidence(
    *,
    ...
    if normalized_form not in {"10-K", "10-KT", "20-F", "40-F"}:
    ...
    for tag in soup.find_all(True):
        concept_name = str(tag.get("name") or "")
        local_concept = concept_name.rsplit(":", 1)[-1].lower() if concept_name else _local_name(tag)
        key = concepts.get(local_concept)
        if key is None:
            continue
        found_any = True
    ...
    if found_any:
        ...
        report_date = _report_date_from_text(soup.get_text("\n", strip=True))
        if report_date is None:
            raise WarehouseRuntimeError("auditor report date not found")
        context, values = sorted(complete)[0]
        return _result(
            accession_number, registrant_cik, normalized_form, document_name,
            audited_period_end, report_date, values["name"], values["location"],
    ...
    report_date = _report_date_from_text(plain)
    ...
    return _result(
        accession_number, registrant_cik, normalized_form, document_name,
        audited_period_end, report_date, signature.group(1), signature.group(2),
